...
export function DeleteRouteModal({ id }: DeleteRouteModalProps) {
  const { mutateAsync: deleteRoute } = client.route.deleteRoute.useMutation({
    onSuccess: () => {
      toast({
    ...
    onError: (error) => {
      toast({
        title: "Error while deleting rotue",
